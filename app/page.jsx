...
export default function DigitalSignage() {
  ...
  const [rotateAngle, setRotateAngle] = useState(0); // 🚀 新增：屏幕旋转角度

  // 🌤️ 实时天气状态
  const [weather, setWeather] = useState({ temp: '--', desc: 'SYNCING', icon: '🌍' });

  // 1. 获取网址后面的 ?screen=X 和 &rotate=90 参数
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // 捕获旋转参数
      const rot = parseInt(params.get('rotate'), 10);
      if (!isNaN(rot)) setRotateAngle(rot);
    }
  }, []);
  ...
  // 🚀 核心逻辑：智能计算网页“躺平”后的宽高尺寸
  const isVertical = rotateAngle === 90 || rotateAngle === -90 || rotateAngle === 270;
  const magicContainerStyle = {
  ...
  return (
    ...
    <div className="w-screen h-screen bg-black overflow-hidden relative">
      ...
      {/* 🎩 会魔法旋转的容器（你在里面随心所欲装扮） */}
      <div style={magicContainerStyle} className="bg-[#F4F1EA] text-[#1A1A1A] p-16 flex flex-col justify-between overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full px-16 py-8 flex justify-between items-center z-50">
          <div className="flex items-center space-x-6 bg-white/50 backdrop-blur-md px-6 py-2 rounded-full border border-white/40 shadow-sm">
            {logoImg ? <img src={logoImg} className="h-16 object-contain" alt="Logo" /> : <span className="text-2xl font-black italic">CGSPACE.NYC</span>}
            <div className="w-px h-4 bg-black/10"></div>
            <span className="font-mono text-[14px] tracking-widest text-zinc-400 uppercase">
        ...
        <div className="flex-1 my-6 relative w-full h-full">
          ...
          <div className={`absolute inset-0 pt-24 flex items-center transition-all duration-1000 ${currentSlide === 0 ? 'opacity-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
            ...
            {/* 右侧：插画小人 (🪄加入了 mix-blend-multiply 魔法) */}
